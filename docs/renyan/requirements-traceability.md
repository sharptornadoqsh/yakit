...
| RY-F-011 | 11 | 漏洞检测能力 | 否 | 客户端支持生成安全漏洞测试报告，格式需支持Word、PDF或HTML。报告内容须包含漏洞详细参数，原始http请求/响应、风险评级及修复建议。 | 充分静态证据 | app/main/reportExport.js 内嵌报告运行资源与数据；report/renderReport.js 保留异常原文并按条目渲染；app/main/handlers/assets.js 统一验证全文、展开详情并输出 HTML、PDF 和 Word 渲染内容；ReportViewerPage.tsx 使用完整 JsonRaw 导出 .docx。2026-09-11 已完成受控样本的真实 Electron 离线与文件内容专项验证，详见验证报告。 | 从实际客户端选取包含全部必填字段、跨页与长报文的数据，逐字段核对离线导出及目标阅读器；专项样本结果不替代实际安装版本和全部阅读器验收。 |

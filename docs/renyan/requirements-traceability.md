...
| 稳定编号 | 源编号 | 继承小类 | 星标 | 需求原文 | 基线状态 | 源码证据与缺失项 | 后续验收方式 |
| --- | ---: | --- | --- | --- | --- | --- | --- |
| RY-NF-025 | 25 | 可靠性要求 | 否 | 产品满足具备稳定运行能力，在多项目并行测试及多用户操作场景下能够持续稳定运行，在长时间运行过程中不出现崩溃或严重性能退化情况。 | 待动态验证 | 当前源码包含并发任务和多项目入口，但静态证据不能证明持续运行、资源占用、崩溃率或性能退化。 | 定义项目数、用户数、任务负载、持续时长和资源上限，执行长时多用户测试并保存性能曲线与异常记录。 |
| RY-NF-026 | 26 | *安全性要求 | 是 | 供应商应提供针对产品的安全检测或风险评估报告，并对报告中发现的问题进行处置。<br>附注原文：注：如采购产品后续需面向互联网提供服务，需要求供应商在产品上线前提供第三方安全检测报告） | 待外部材料 | 该项要求供应商或第三方报告及问题处置记录，不能从源码生成或推定。 | 收集报告原件、检测范围、报告日期、问题清单、处置证据；互联网服务上线前另行核验第三方报告。 |
...
| RY-NF-028 | 28 | *安全性要求 | 是 | 产品应提供用户权限管理功能，控制用户根据既定的访问权限和访问方式进行访问。 | 部分静态证据 | RoleAdminPage.tsx 提供角色与操作权限管理入口；前端接口存在不能证明全部服务端资源均实施授权检查。 | 建立角色权限矩阵，用不同角色直接访问界面和接口，验证允许、拒绝、越权和权限变更生效。 |
| RY-NF-029 | 29 | *安全性要求 | 是 | 用户初始设置或变更静态密码时，对其进行复杂度检查 | 部分静态证据 | app/renderer/src/main/src/components/ConfigPrivateDomain/ConfigPrivateDomain.tsx 对密码实施长度及字符类别校验，并触发密码重置流程；未证明所有初始设置和变更入口共用该校验。 | 枚举全部密码建立与变更入口，对长度、大小写、数字、特殊字符及边界组合执行一致性测试。 |
| RY-NF-030 | 30 | *安全性要求 | 是 | 不应在配置文件、数据库、程序代码等载体中明文存储用户密码。 | 存在冲突风险 | ConfigPrivateDomain.tsx 使用 base64 编码保存 pwd，并在读取时执行 base64-decode。可逆编码不能提供密码不可恢复性，且当前前端仓库不能证明远端存储形态，因此不能判定符合要求。 | 清点配置、数据库、日志和远端键值中的凭据；确认不含明文或可逆编码密码，并以不可逆口令派生结果和迁移记录作为证据。 |
| RY-NF-031 | 31 | *安全性要求 | 是 | 对于CVE、CNVD等公开渠道通告的产品本身的漏洞，供应商应提供已知漏洞修复的相关说明；对于产品使用的第三方或开源组件的安全漏洞，供应商应提供已知漏洞修复的相关说明。 | 待外部材料 | 当前仓库不能代表供应商在指定日期的完整漏洞清单、组件清单、修复公告和交付说明。 | 获取软件物料清单、产品与依赖漏洞清单、修复版本、公告和例外说明，按发布日期与交付版本逐项核验。 |

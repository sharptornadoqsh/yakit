...
2026-09-11 按用户明确的修复请求更新 RY-F-011 的实现证据，不修改原文、编号或最初基线提交。
风险结果与安全测试报告的 HTML 交付改为内嵌资源和数据；异常条目保留原文，空正文明确停止导出。
PDF 与 Word 使用全文渲染，展开详情并保留长报文和图形；Word 扩展名改为 `.docx`。

本轮证据来自人工构造数据、隔离的真实 Electron IPC 和断网渲染，以及生成文件的内容和图片复查。
测试没有读取或改写实际风险数据库，没有替换正在运行的安装客户端；目标 Office/WPS 阅读器及其他操作系统
仍须在对应环境验收。具体命令、输出与交付摘要保存在本仓库 `.Codex/report-export-repair-20260911/`。

...
| SHA-256 | 7C0FB6DA86E83457B23510F097554A149A2CDC4B4E649B28A44C7ED7CBC91783 |
| 工作表数量 | 3 |
| 有效工作表 | Sheet1 |
| 主表范围 | 33 行、4 列 |
| 编号需求 | 31 项 |
...
2026 年 7 月 14 日复验发现工作簿字节摘要和修改时间发生变化。只读提取的三十一项描述及第二十六项附注与需求追踪表逐项比较，差异数量为零；稳定编号、分类、星标和需求原文不变，因此沿用当前基线编号并更新需求源摘要。
...
| 本地与跟踪差异 | 左侧 0、右侧 0 |
| 代码索引 | 1712 个文件、32839 个节点、93575 条边，状态为最新 |
...
| 部分静态证据 | 8 | RY-F-013、RY-F-015、RY-F-018 至 RY-F-021、RY-NF-028、RY-NF-029 |
| 未发现直接证据 | 2 | RY-F-017、RY-F-024 |
| 待动态验证 | 1 | RY-NF-025 |
| 待外部材料 | 3 | RY-NF-026、RY-NF-027、RY-NF-031 |
| 存在冲突风险 | 1 | RY-NF-030 |
...
- 根目录和两个渲染器依赖目录均未安装，项目 Prettier 命令不可用。
- 本次只修改文档，没有应用行为或构建配置变化，因此未执行应用测试、类型检查和产品构建。

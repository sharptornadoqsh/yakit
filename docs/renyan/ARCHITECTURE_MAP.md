...
| 层次 | 入口 | 输入 | 输出与职责 |
| --- | --- | --- | --- |
| 兼容数据层 | `product/engine-compatibility.json` | 客户端版本、平台和架构 | 版本边界、能力检查名称、预置文件与摘要 |
| 主侧事务层 | `app/main/engineLifecycle.js` | 下载文件、预置压缩包、目标引擎、期望摘要 | 摘要计算、精确提取、原子替换、中断恢复与最后可用副本 |
| 主侧接入层 | `upgradeUtil.js`、两个 `yaklangAndYakit.js` | 既有进程间通信请求 | 显式下载、安装、预置恢复、手工安装、启动前恢复与阶段事件 |
| 构建约束层 | `packageScript/buildHook/before-pack.js` | 目标平台、架构与兼容清单 | 打包前验证实体预置包和压缩包摘要 |
| 启动决策层 | `StartupPage/engineLifecycle.ts` | 用户模式、本地工件、兼容结果与预置工件 | 十四种状态、纯转换函数与启动决策 |
| 本地能力层 | `LocalEngine` | 本地能力结果、本地版本与兼容清单 | 离线兼容判定、阻断原因与启动凭据 |
| 初始化呈现层 | `EngineLifecyclePanel` | 生命周期状态、步骤、进度与错误 | 安装、远程、手工、重试和诊断入口 |
| 主界面更新层 | `UILayout`、`engineUpdate.ts`、`RenyanStatusBar` | 本地引擎连接状态、当前与在线版本 | 非阻塞后台更新状态和离线“未检查”状态 |
...
├─ 远程模式 → 读取远程参数 → 连接远程引擎
└─ 本地模式
   ├─ 活动引擎存在 → 恢复未完成安装 → 本地能力与兼容清单检查 → 启动
   ├─ 活动引擎缺失且可信预置包存在 → 双摘要验证 → 提取临时文件 → 原子安装 → 启动
   └─ 活动引擎与可信预置包均缺失 → 等待用户选择安装、远程、手工安装或诊断
```

本地版本低于推荐版本但能力检查通过时继续启动。低于未来已知最低版本或能力检查失败时进入不兼容状态。初始化窗口不执行在线最新版本查询；主界面可用且本地引擎已连接后才执行后台查询。
...
任何失败只清理本次临时文件。活动文件已移动时恢复 `.rollback`；应用下次启动还会读取安装标记，并从回退文件或 `.last-good` 恢复。版本缓存采用相同的下载、校验和替换服务。

### 四、协议边界

阶段三新增的生命周期服务位于客户端内部。现有引擎远程过程调用名称、远程连接参数、进程启动方式、数据库、插件接口、企业类别与许可证条件没有改变。在线地址仍由既有网络模块生成，摘要校验没有被绕过。

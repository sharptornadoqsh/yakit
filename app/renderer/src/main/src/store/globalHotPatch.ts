...
//
// 你可以通过观察请求头中是否出现 X-RuiYan-Global-HotPatch 来确认是否生效

hijackHTTPRequest = func(isHttps, url, req, forward, drop) {
    req = poc.AppendHTTPPacketHeader(req, "X-RuiYan-Global-HotPatch", "1")
    forward(req)
}
...
//   3. hijackSaveHTTPFlow: 入库时确保 Request/Response 都是明文（便于在 RuiYan 历史里直接看）
...
说明:
  ...
  - 当本模板被复制到 RuiYan 全局热加载窗口使用时，YAK_MAIN = false，下方测试代码不会执行

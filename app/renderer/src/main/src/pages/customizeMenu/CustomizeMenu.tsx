...
const CustomizeMenu: React.FC<CustomizeMenuProps> = React.memo((props) => {
  ...
  const editCurrentFirstMenu = useMemoizedFn((value: string) => {
    ...
    const index = menuData.findIndex((ele) => ele.id === currentFirstMenu?.id && ele.label === currentFirstMenu?.label)
    if (index === -1) return
  ...
  const updateData = useMemoizedFn((subMenuList: EnhancedCustomRouteMenuProps[]) => {
    ...
    const index = menuData.findIndex((item) => item.id === currentFirstMenu.id && item.label === currentFirstMenu.label)
    if (index === -1) return
    menuData[index] = {
      ...menuData[index],
  ...
  const onSaveLocal = useMemoizedFn(() => {
    ...
    const menus = isCommunityEdition() ? publicConvertDatabase(menuData) : privateConvertDatabase(menuData, patternMenu)
    setAddLoading(true)
    // 过滤出用户删除的系统内定菜单
...
const FirstMenu: React.FC<FirstMenuProps> = React.memo((props) => {
  ...
  const onDragEnd = useMemoizedFn((result) => {
    ...
    if (result.source.droppableId === 'droppable1' && result.destination.droppableId === 'droppable1') {
      const menuList: EnhancedPrivateRouteMenuProps[] = reorder(menuData, result.source.index, result.destination.index)
                ...
                {subMenuData.length === 0 && (
                  <div className={style['second-menu-no-data']}>
                    ...
                    <div>
                      ...
                      <div className={style['second-menu-text']}>{t('CustomizeMenu.SecondMenu.addFeatureHint')}</div>
                    </div>
                  </div>
                )}
                {provided.placeholder}
        ...
        return (
          <div {...provided.droppableProps} ref={provided.innerRef} className={style['system-function-list']}>
            {systemRouteMenuData.map((item, index) => {
              const isDragDisabled = props.subMenuData.findIndex((i) => i.menuName === item.menuName) !== -1
              return (
                <Draggable
                  key={item.label}
  ...
  return (
    ...
    >
      <div className={style['display-flex']}>
        ...
        <span className={classNames(style['menu-label'], { [style['menu-item-isDragDisabled']]: isDragDisabled })}>
          ...
          <div className={style['plugin-local-list']} {...provided.droppableProps} ref={provided.innerRef}>
            <RollingLoadList<YakScript>
              ...
              renderRow={(data: YakScript, index) => {
                const isDragDisabled = props.subMenuData.findIndex((i) => i.yakScripName == data.ScriptName) !== -1
                return (
                  <Draggable
                    key={data.Id}

...
export const AddPluginMenuContent: React.FC<AddPluginMenuContentProps> = (props) => {
  ...
  const onFinsh = useMemoizedFn((values: any) => {
    if (!script) {
      yakitNotify('error', 'No RuiYan Module Selected')
      return
    }
    if (loading) return
    setLoading(true)
  ...
  return (
    <div className={styles['add-plugin-menu-content']}>
      <Form form={form} layout="vertical" onFinish={onFinsh}>
        ...
        <Form.Item
          label={t('FuncTemplate.menuGroup')}
          name="Group"
          rules={[{ required: true, message: t('YakitForm.requiredField') }]}
        >
  ...
  return (
    <div className={classNames(styles['hub-detail-wrapper'], wrapperClassName)}>
      ...
      <div className={styles['author-wrapper']}>
        ...
        <div className={classNames(styles['text-style'], { [styles['constant-wrapper']]: !infoExtra })}>
          {t('HubExtraOperate.updatedAt', { date: formatDate(updated_at) })}
        </div>

...
export const PluginLogOpt: React.FC<PluginLogOptProps> = memo((props) => {
  ...
  const additional = useMemo(() => {
    ...
    if (key === 'update' || key === 'applyMerge' || key === 'mergeNoPass' || key === 'checkNoPass') {
      ...
      return (
        <div className={classNames(styles['description-style'], 'yakit-content-multiLine-ellipsis')}>{content}</div>
      )
    }
    if (key === 'comment') {
      ...
      return (
        <>
          {!!text && (
            <div className={classNames(styles['description-style'], 'yakit-content-multiLine-ellipsis')} title={text}>
              {text}
            </div>
          )}
          {isImgs && (
      ...
      return (
        <>
          <div className={styles['reply-style']}>
            ...
            >
              {!!replyText && (
                <div className={classNames(styles['content-style'], 'yakit-content-single-ellipsis')} title={replyText}>
                  {replyText}
                </div>
              )}

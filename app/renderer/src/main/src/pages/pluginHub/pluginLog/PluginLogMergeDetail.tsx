...
export const PluginLogMergeDetail: React.FC<PluginLogMergeDetailProps> = memo((props) => {
  ...
  const fetchPluginInfo = useMemoizedFn(async () => {
    if (!uuid || !id) {
      yakitNotify('error', t('PluginLogMergeDetail.infoMissingRetry'))
      return
    }
    if (loading) return
    ...
    httpFetchMergePluginDetail({ uuid: uuid, up_log_id: id })
      .then(async (res) => {
        if (res) {
          ...
          if (!oldInfo) {
            yakitNotify('error', t('PluginLogMergeDetail.compareDataMissingRetry'))
            onCancel()
            return
          }
          ...
          yakitNotify('error', t('PluginLogMergeDetail.modifyContentEmptyRetry'))
          onCancel()
        }
      })
      .catch((err) => {
  ...
  const changePRInfo: (isPass: boolean, reason?: string) => Promise<API.PluginsLogsDetail> = useMemoizedFn(
    (isPass, reason) => {
      return new Promise(async (resolve, reject) => {
        if (prInfo) {
          if (!formRef.current) {
            yakitNotify('error', t('PluginLogMergeDetail.baseInfoMissingRetry'))
            reject()
            return
          }
          ...
          yakitNotify('error', t('PluginLogMergeDetail.modifyInfoMissingRetry'))
          reject()
        }
      })
    },
    ...
    <>
          ...
          <div className={styles['plugin-log-merge-detail-wrapper']}>
            ...
            >
              <div className={styles['diff-header']}>
                <YakitTag color={pluginTypeTag?.color as YakitTagColor}>{pluginTypeTag?.name || ''}</YakitTag>
            ...
            >
              <div className={styles['baseinfo-body']} style={{ background: 'var(--Colors-Use-Neutral-Bg-Hover)' }}>
      ...
      >
        <Form form={noPassForm}>
          <Form.Item
            label=""
            name="noPassReason"
            rules={[{ required: true, message: t('PluginLogMergeDetail.noMergeReasonRequired') }]}
          >
            <YakitInput.TextArea
              placeholder={t('PluginLogMergeDetail.noMergeReasonPlaceholder')}
      ...
      >
        <CodeScoreModule
          ...
          isStart={pass}
          successWait={10}
          successHint={t('PluginLogMergeDetail.scoreSuccessHint')}
          failedHint={t('PluginLogMergeDetail.scoreFailedHint')}
          specialHint={t('PluginLogMergeDetail.scoreSpecialHint')}
          specialBtnText={t('PluginLogMergeDetail.scoreSpecialBtn')}
          specialExtraBtn={
    ...
    return (
      <Form className={styles['plugin-base-info-form']} form={form} layout="vertical">
        <Form.Item
          label={
            <>
              {t('PluginLogMergeDetail.scriptType')}
              <span className="form-item-required">*</span>:
            </>
          }
          name="Type"
          rules={[{ required: true, message: t('PluginLogMergeDetail.scriptTypeRequired') }]}
        >
        ...
        <Form.Item
          label={
            <>
              {t('PluginLogMergeDetail.pluginName')}
              <span className="form-item-required">*</span>:
            </>
          }
          name="ScriptName"
          ...
          rules={[
            {
              validator: async (_, value) => {
                if (!value || !value.trim())
                  return Promise.reject(new Error(t('PluginLogMergeDetail.pluginNameRequired')))
                if (value.trim().length > 100) return Promise.reject(new Error(t('PluginLogMergeDetail.nameMaxLength')))
              },
            },
          ]}
        >
        ...
        <Form.Item label={t('PluginLogMergeDetail.description')} name="Help">
          <YakitInput.TextArea rows={2} placeholder={t('YakitInput.please_enter')} disabled={allDisabled} />
        </Form.Item>
        ...
        >
          <Form.Item noStyle name="Tags" rules={[{ required: true, message: t('PluginLogMergeDetail.tagsRequired') }]}>
            <YakitSelect
        ...
        {EnablePluginSelector && (
          <Form.Item
            ...
            label={
              <>
                {t('PluginLogMergeDetail.linkedPluginType')}
                <span className="form-item-required">*</span>:
              </>
            }
            required={true}
            rules={[{ required: true, message: t('PluginLogMergeDetail.linkedPluginTypeRequired') }]}
          >
            <YakitSelect
    ...
    httpFetchMergePluginDetail({ uuid: uuid, up_log_id: id })
      .then(async (res) => {
        if (res) {
          ...
          yakitNotify('error', t('PluginLogMergeDetail.fetchEmpty'))
          onCancel()
        }
      })
      .catch(() => {
